...
:root {
    ...
    --gold-soft: #eadfc4;
    --rose: #c08579;
    --shadow: 0 10px 32px rgba(87, 74, 40, 0.07);
    --shadow-sm: 0 3px 14px rgba(87, 74, 40, 0.06);
}
.stApp {
...
h1, h2, h3 {
    color: var(--sage-deep); font-family: "Fraunces", ui-serif, Georgia, serif;
    letter-spacing: -0.01em; font-weight: 620;
}
h1 { font-size: 2.5rem; }
p, label, .stMarkdown, span, div { font-family: "Inter", ui-sans-serif, system-ui, sans-serif; }
p, label { color: var(--ink); }
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #f3ede0 0%, #ece4d2 100%);
    border-right: 1px solid var(--line);
}
[data-testid="stSidebar"] h3 { color: var(--sage-deep); }
[data-testid="stMetric"] {
    background: var(--card); border: 1px solid var(--line); border-radius: 18px;
    padding: 18px 20px; box-shadow: var(--shadow-sm);
    transition: box-shadow .2s ease, transform .2s ease;
}
[data-testid="stMetric"]:hover { box-shadow: var(--shadow); transform: translateY(-1px); }
[data-testid="stMetricLabel"] { color: var(--muted); font-weight: 500; }
[data-testid="stMetricValue"] { color: var(--sage-deep); font-family: "Fraunces", serif; }
.soft-card {
...
.eyebrow {
    color: var(--gold); text-transform: uppercase; font-size: .74rem;
    letter-spacing: .14em; font-weight: 700;
}
.hero-copy { color: var(--muted); font-size: 1.08rem; max-width: 850px; margin-top: -.3rem; line-height: 1.55; }
.score-number {
    font-size: 3rem; line-height: 1; font-weight: 650;
    font-family: "Fraunces", serif; color: var(--ink);
}
.score-label { color: var(--muted); font-size: .87rem; margin-top: .4rem; }
.pill {
    display: inline-block; border-radius: 999px; padding: 6px 12px; margin: 3px 5px 3px 0;
    font-size: .78rem; font-weight: 500; background: var(--gold-soft); color: var(--sage-deep);
    border: 1px solid #ddd0ab;
}
.pill-risk { background: #f6e8e3; color: #93544a; border-color: #ecd3cb; }
.source-badge {
    display:inline-block; padding: 5px 10px; border-radius: 8px; font-size: .72rem;
    margin-right: 5px; font-weight: 600;
}
.source-live { background: linear-gradient(90deg, #e3efe0, #eef5e6); color: #3d6d47; }
.source-seed { background: #f1ece0; color: #8a7749; }
.small-note { color: var(--muted); font-size: .8rem; line-height: 1.5; letter-spacing: .01em; }
hr { border-color: var(--line); }
.stButton > button, .stDownloadButton > button {
    border-radius: 12px; border: 1px solid var(--sage); background: var(--card); color: var(--sage-deep);
    font-weight: 600; transition: all .2s ease;
}
.stButton > button:hover, .stDownloadButton > button:hover {
    border-color: var(--sage-deep); color: #fff; background: var(--sage-dark);
    box-shadow: var(--shadow-sm);
}
button[kind="primary"] {
    background: linear-gradient(120deg, var(--sage-dark), var(--sage-deep)) !important;
    border: none !important; color: #fff !important;
}
button[kind="primary"]:hover { box-shadow: var(--shadow) !important; filter: brightness(1.06); }
...
.stTabs [data-baseweb="tab"] {
    background: transparent; border-radius: 10px 10px 0 0; padding: .6rem 1.1rem;
    color: var(--muted); font-weight: 500;
}
.stTabs [aria-selected="true"] {
    background: var(--card); color: var(--sage-deep); font-weight: 650;
...
@st.cache_data(ttl=60 * 60 * 24, show_spinner=False)
def fetch_fda_signals(disease: str) -> dict[str, Any]:
    """Best-effort FDA signal: openFDA does not expose the official Orphan
    Drug Designation database as a documented REST endpoint, so this counts
    approved drug labels whose indications mention the disease instead. It is
    a proxy for 'an approved product exists that treats this,' not an
    authoritative orphan-designation count."""
    response = HTTP.get(
        "https://api.fda.gov/drug/label.json",
        params={"search": f'indications_and_usage:"{disease}"', "limit": 10},
        timeout=15,
    )
    if response.status_code == 404:
        # openFDA returns 404 for zero matches rather than an empty result set.
    ...
    return {"count": int(total), "products": brands[:8]}
...
def refresh_live_data(disease: str, seed: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    ...
    try:
        fda = fetch_fda_signals(disease)
        data["fda_label_count"] = fda["count"]
        data["fda_label_products"] = fda["products"]
        live_fields.append("openFDA labels")
    except Exception as exc:
        errors.append(f"openFDA labels ({_friendly_error(exc)})")
...
# Scoring and charts
# -----------------------------------------------------------------------------
DEFAULT_WEIGHTS = {"bio": 25, "feasibility": 25, "population": 15, "regulatory": 20, "commercial": 15}
DIMENSION_LABELS = {
    "bio": "Biological validation",
    "feasibility": "Trial feasibility",
    "population": "Patient population",
    "regulatory": "Regulatory clarity",
    "commercial": "Commercial incentive",
}
WEIGHTS = {k: v / 100 for k, v in DEFAULT_WEIGHTS.items()}


def opportunity_score(data: dict[str, Any], weights: dict[str, float] | None = None) -> float:
    w = weights or WEIGHTS
    return round(sum(data[key] * weight for key, weight in w.items()), 1)


def score_breakdown(data: dict[str, Any], weights: dict[str, float] | None = None) -> pd.DataFrame:
    """Show exactly how each dimension contributed to the final score -
    the 'why this score' view."""
    w = weights or WEIGHTS
    rows = []
    for key, label in DIMENSION_LABELS.items():
        rows.append({
            "Dimension": label,
            "Score (1-10)": data[key],
            "Weight": f"{w[key] * 100:.0f}%",
            "Contribution": round(data[key] * w[key], 2),
        })
...
def radar_chart(data: dict[str, Any], name: str) -> go.Figure:
    ...
    fig = go.Figure(go.Scatterpolar(
        ...
        fillcolor="rgba(79,115,87,0.18)", line=dict(color="#4f7357", width=2.5), name=name,
    ))
    fig.update_layout(
        polar=dict(
            bgcolor="#fffdf8",
            radialaxis=dict(range=[0, 10], tickvals=[2, 4, 6, 8, 10], gridcolor="#e7e0cf", linecolor="#e7e0cf", tickfont=dict(color="#9a9584")),
            angularaxis=dict(gridcolor="#e7e0cf", linecolor="#e7e0cf", tickfont=dict(color="#4f5648", size=11)),
        ),
...
def compare_chart(a: dict[str, Any], a_name: str, b: dict[str, Any], b_name: str) -> go.Figure:
    ...
    for data, name, color, fill in [
        (a, a_name, "#4f7357", "rgba(79,115,87,.16)"),
        (b, b_name, "#c69a5c", "rgba(198,154,92,.16)"),
    ]:
        vals = [data["bio"], data["feasibility"], data["population"], data["regulatory"], data["commercial"]]
        fig.add_trace(go.Scatterpolar(r=vals + [vals[0]], theta=labels + [labels[0]], fill="toself", fillcolor=fill, line=dict(color=color, width=2.5), name=name))
    fig.update_layout(
        polar=dict(bgcolor="#fffdf8", radialaxis=dict(range=[0, 10], gridcolor="#e7e0cf"), angularaxis=dict(gridcolor="#e7e0cf")),
...
with st.sidebar:
    ...
    st.caption("Adjust how much each dimension counts. The score recalculates live everywhere in the app.")
    if st.button("Reset weights to default", use_container_width=True):
        for key in DEFAULT_WEIGHTS:
            st.session_state[f"w_{key}"] = DEFAULT_WEIGHTS[key]
        st.rerun()
    raw_weights = {}
    for key, label in DIMENSION_LABELS.items():
        raw_weights[key] = st.slider(label, 0, 50, DEFAULT_WEIGHTS[key], 5, key=f"w_{key}")
    total_w = sum(raw_weights.values()) or 1
    WEIGHTS = {k: v / total_w for k, v in raw_weights.items()}
    if total_w != 100:
        st.caption(f"Raw weights sum to {total_w}%, normalized to 100% for scoring.")
    st.markdown("---")
    ...
    st.caption("openFDA drug label search, best-effort signal")
    st.caption("FDA orphan designation database, linked for manual verification")
    st.markdown("---")
...
with analyze_tab:
    ...
    if refresh:
        with st.spinner("Querying public biomedical sources..."):
            ...
            st.session_state.history.setdefault(selected, []).append({
                "Refreshed": live["last_refreshed"],
                "Score": opportunity_score(live),
                "Active trials": live["trials"],
                "PubMed records": live["pubmed_citations"],
                "NIH funding": live["nih_funding"],
            })
...
""",
        ...
        st.caption("Each dimension's score × its sidebar weight = its contribution to the total.")
        breakdown_df = score_breakdown(data, WEIGHTS)
        st.dataframe(breakdown_df, hide_index=True, use_container_width=True, height=212)
        b = barriers(data)
        if b:
    ...
    with e3:
        fda_count = data.get("fda_label_count")
        fda_products = data.get("fda_label_products") or []
        fda_line = f"<p><b>{fda_count}</b> FDA drug labels mentioning this condition</p>" if fda_count is not None else "<p>Not refreshed</p>"
        fda_products_line = f"<p>{', '.join(fda_products)}</p>" if fda_products else "<p class='small-note'>No matching labeled products found.</p>" if fda_count is not None else ""
        st.markdown(
            f"""<div class="soft-card"><div class="eyebrow">FDA signal (best-effort)</div>
            {fda_line}
            {fda_products_line}
            <p><b>{data['approved_therapies']}</b> modeled approved-therapy count</p></div>""",
            unsafe_allow_html=True,
        )
    with e4:
    ...
    if len(history) > 1:
        st.markdown("#### Score over refreshes")
        st.caption("Each refresh is snapshotted, so you can see how this disease's evidence has moved across your session.")
        hist_df = pd.DataFrame(history)
        trend = px.line(hist_df, x="Refreshed", y="Score", markers=True)
        trend.update_traces(line=dict(color="#4f7357", width=2.5), marker=dict(color="#c69a5c", size=8))
        trend.update_layout(
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#fffdf8", height=280,
            margin=dict(t=20, b=20, l=20, r=20), yaxis=dict(range=[0, 10], gridcolor="#e7e0cf"),
            xaxis=dict(gridcolor="#e7e0cf"), font=dict(family="Inter, sans-serif"),
        )
        st.plotly_chart(trend, use_container_width=True, config={"displayModeBar": False})
        st.dataframe(hist_df, hide_index=True, use_container_width=True)

    st.markdown("#### Export this profile")
    ex1, ex2 = st.columns(2)
    with ex1:
        brief_lines = [
            f"{selected} — Translation Intelligence Brief",
            f"Generated {datetime.now().astimezone().strftime('%b %d, %Y at %I:%M %p')}",
            "",
            f"Gene or pathway: {data['gene']}",
            f"Category: {data['category']}",
            f"Estimated U.S. prevalence: {data['prevalence']:,}",
            f"Translation score: {score}/10 ({risk})",
            f"Evidence basis: {data['data_source']}",
            "",
            "Dimension scores (1-10):",
        ]
        for key, label in DIMENSION_LABELS.items():
            brief_lines.append(f"  - {label}: {data[key]}")
        if b:
            brief_lines += ["", "Priority barriers:"] + [f"  - {item}" for item in b]
        brief_lines += [
            "",
            f"Active trials: {data['trials']} | Registered studies: {data.get('registered_trials', data['trials'])} | Highest phase: {data['phase_max']}",
            f"PubMed records: {data['pubmed_citations']:,} | NIH funding (5y): ${data['nih_funding']:,}",
            f"FDA labels mentioning condition: {data.get('fda_label_count', 'Not refreshed')}",
            "",
            "Scores are research prioritization signals, not clinical or investment advice.",
        ]
        st.download_button(
            "Download text brief",
    ...
    with ex2:
        try:
            chart_png = radar_chart(data, selected).to_image(format="png", width=900, height=700, scale=2)
            st.download_button(
                "Download radar chart (PNG)",
                data=chart_png,
                file_name=f"{selected.lower().replace(' ', '_')}_radar.png",
                mime="image/png",
                use_container_width=True,
            )
        except Exception:
            st.caption("Chart image export isn't available in this environment.")
...
with portfolio_tab:
    ...
    scatter.update_layout(
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#fffdf8", legend_title_text="Category",
        margin=dict(t=25, b=20, l=20, r=20), height=510,
        font=dict(family="Inter, sans-serif", color="#2b332a"),
    )
    scatter.update_xaxes(gridcolor="#e7e0cf")
    scatter.update_yaxes(gridcolor="#e7e0cf")
...
with methods_tab:
    ...
    st.markdown(
        """
The platform combines five 1-to-10 dimensions into a weighted translation score. Default weights are shown below, but you can adjust them in the sidebar — the score recalculates live across every tab, so you can see how sensitive a disease's ranking is to your assumptions:

- **Biological validation, 25% default:** strength and specificity of the disease mechanism and therapeutic rationale.
- **Trial feasibility, 25% default:** enrollment practicality, endpoint quality, natural-history readiness, and operational burden.
- **Patient population, 15% default:** relative ability to identify and recruit an addressable population.
- **Regulatory clarity, 20% default:** precedent, surrogate endpoint acceptance, and pathway maturity.
- **Commercial incentive, 15% default:** addressable market, competition, development cost, and likely reimbursement support.

The 100-disease catalog starts with modeled baseline values so every screen remains usable without internet access. Pressing **Refresh live evidence** replaces trial counts, literature counts, recent NIH funding, and FDA label signals with current API results for the selected disease, and snapshots the result so you can track how a disease's score moves across refreshes in the same session. Those live fields provide evidence inputs, while the five translation dimensions remain editable model assumptions in this prototype. The "Why this score" table on the analyzer tab shows exactly how each dimension and weight combined to produce the final number.
"""
    )
...
- The FDA signal counts approved drug labels (via openFDA) whose indications mention the disease name — a best-effort proxy for "an approved product treats this," not an authoritative orphan-designation count. FDA does not expose the official Orphan Drug Designation database as a documented public REST endpoint, so the searchable database is also linked for manual verification.
"""
    )
    st.markdown("### Illustrative validation check")
    st.caption(
        "A lightweight, non-statistical sanity check: does the model's directional call match well-documented real-world outcomes for diseases with mature or well-known programs? "
        "This is illustrative, not a formal back-test — it doesn't establish predictive accuracy, only that the model isn't obviously miscalibrated on cases we can verify by hand."
    )
    validation_rows = [
        ("Spinal Muscular Atrophy", "Lower risk", "Approved therapies (Spinraza, Zolgensma, Evrysdi) reached patients", "Consistent"),
        ("Duchenne Muscular Dystrophy", "Lower risk", "Approved exon-skipping and gene therapies exist", "Consistent"),
        ("Pompe Disease", "Lower risk", "Enzyme replacement therapies approved and established", "Consistent"),
        ("Cystic Fibrosis", "Lower risk", "CFTR modulators transformed outcomes for most genotypes", "Consistent"),
        ("Hereditary Transthyretin Amyloidosis", "Lower risk", "Multiple approved gene-silencing and stabilizer therapies", "Consistent"),
        ("Visceral Myopathy", "Higher risk", "No approved therapy; ultra-rare, mechanism still maturing", "Consistent"),
        ("Barth Syndrome", "Higher risk", "No approved therapy; very small addressable population", "Consistent"),
        ("Canavan Disease", "Higher risk", "No approved therapy despite known single-gene cause", "Consistent"),
    ]
    st.dataframe(
        pd.DataFrame(validation_rows, columns=["Disease", "Model call", "Real-world benchmark", "Directional match"]),
        hide_index=True, use_container_width=True,
    )
    st.caption("Diseases were chosen because their real-world development outcomes are well documented, not because they were guaranteed to match the model — this list should grow as more cases are checked.")
