...
def fetch_clinical_trials(disease: str) -> dict[str, Any]:
    ...
    for study in studies:
        ...
        max_phase = max(max_phase, *(phase_map.get(p, 0) for p in phases))
        if len(recent) < 8:
            recent.append({
...
def refresh_live_data(disease: str, seed: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    ...
    try:
        ...
        errors.append(f"ClinicalTrials.gov: {type(exc).__name__}")
    try:
    ...
    try:
        ...
        errors.append(f"NIH RePORTER: {type(exc).__name__}")
